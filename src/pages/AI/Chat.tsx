...
const Chat = () => {
  ...
  const [isShowChatMenu, setIsShowChatMenu] = useState<boolean>(false);
  const [searchUser, setSearchUser] = useState<string>("");
  const [isShowUserChat, setIsShowUserChat] = useState<boolean>(false);
  const [selectedUser, setSelectedUser] = useState<User | null>(null);
  const [textMessage, setTextMessage] = useState<string>("");
  const [filteredItems, setFilteredItems] = useState<User[]>(contactList);
  useEffect(() => {
    setFilteredItems(() => {
  ...
  //     const user: User | undefined = contactList.find(
  //       (d) => d.userId === selectedUser.userId
  //     );
  //     if (user) {
  //       // Add the user's message
  //       user.messages.push({
  //         fromUserId: 1, // User ID
  ...
  //       setTextMessage("");
  //       setFilteredItems([...contactList]); // Update the filtered items
  //       scrollToBottom();

  //       // Set a timeout to simulate the robot's response
  //       setTimeout(() => {
  //         const robotResponse =
  //           "Thank you for your message! How can I assist you further?";
  //         user.messages.push({
  //           fromUserId: 0, // Robot ID
  //           toUserId: user.userId,
  //           text: robotResponse,
  //         });

  //         setFilteredItems([...contactList]); // Update the filtered items again to include the robot's response
  //         scrollToBottom(); // Scroll to the bottom to show the new message

  //         console.log("Chat History:", user.messages);
  //       }, 3000); // Delay the robot's response by 1 second
  //     }
  //   }
  // };
  ...
  const sendMessage = async () => {
    if (textMessage.trim() && selectedUser) {
      ...
      if (user) {
        ...
        // Log the chat history after the user's message
        console.log("Chat History:", user.messages);

        // Prepare the chat history for the bot response
        const history = user.messages.map((msg) => ({
          role: msg.fromUserId === 0 ? "model" : "user", // Change "bot" to "model"
          text: msg.text,
        }));

        // Get the bot's response
        await generateBotResponse(history, user);
      }
    }
  };

  const generateBotResponse = async (history, user) => {
    ...
    try {
      ...
      const botResponseText =
        data.candidates[0]?.content?.parts[0]?.text || "No response"; // Adjust based on actual response structure
      ...
      // Log the chat history after the bot's response
      console.log("Chat History:", user.messages);
    } catch (error) {
      console.log(error);
    }
        ...
        >
          <div className="flex justify-between items-center">
            <div className="flex items-center">
              ...
              <div className="mx-3">
                <p className="mb-1 font-semibold">FZ</p>
        ...
        <div className="panel p-0 flex-1">
          ...
          {isShowUserChat && selectedUser ? (
            <div className="relative h-full">
              ...
              <div className="space-y-5 p-4 sm:pb-0 pb-[68px] sm:min-h-[300px] min-h-[400px] overflow-hidden">
                <div className="overflow-y-auto h-full max-h-[calc(82vh-200px)]">
